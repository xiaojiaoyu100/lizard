...
func cpuproc(logger ratelimiter.Logger) {
	ticker := time.NewTicker(time.Millisecond * 250)
	defer func() {
		ticker.Stop()
		if err := recover(); err != nil {
			ratelimiter.SafeLog(logger, func(logger ratelimiter.Logger) {
				logger.Errorf("rate.limit.cpuproc() err(%+v)", err)
			})
			go cpuproc(logger)
		}
	}()
...
func newLimiter(conf *Config) ratelimiter.Limiter {
	if conf == nil {
		conf = defaultConf
	}
	size := conf.WinBucket
...
func (g *Group) Get(key string) ratelimiter.Limiter {
	limiter := g.group.Get(key)
	return limiter.(ratelimiter.Limiter)
}
